        ...
        if (openai) {
            ...
            return (openai = new OpenAI$1({
                ...
                fetch: async (url, init) => fetch(`${authoringBase}/api/2/plugin/script/plugins/org/craftercms/openai/key?siteId=${siteId}`, {
                    ...
                    return fetch("http://localhost:8080/api/api.json?crafterSite=ed");
...
function ChatGPT(props) {
    // region const { ... } = props;
    const { sxs, model = 'gpt-3.5-turbo', userName, scrollToReply = true, initialMessages, aiAvatarColour = '#19c37d', extraActions, emptyStateOptions = [
        {
            id: 'useCasualTone',
